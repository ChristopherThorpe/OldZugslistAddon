...
function TFW_AddMessage(msg, sender, color, forwarder, force, counter)
	...
	for i=1, NUM_CHAT_WINDOWS do
		...
		if show then
			...
			frame:AddMessage("[TFW"..(forwarder and ":"..forwarder or "").."] "..(sender and "|Hplayer:"..sender..(counter and ":"..counter or "").."|h["..(color and "|cff"..color or "")..sender..(color and "|r" or "").."]|h: " or "")..msg, r, g, b, chatInfo.id)
		end
	end
	if(force and not one) then
		DEFAULT_CHAT_FRAME:AddMessage("[TFW"..(forwarder and ":"..forwarder or "").."] "..(sender and "|Hplayer:"..sender..(counter and ":"..counter or "").."|h["..(color and "|cff"..color or "")..sender..(color and "|r" or "").."]|h: " or "")..msg, r, g, b, chatInfo.id)
	end
end
...
local function SendCmd(cmd, flag)
	if(tfwChNumber) then
		...
		SendChatMessage(cmd, "CHANNEL", LFW_LANGUAGE, tfwChNumber);
...
SlashCmdList["TFW"] = function(cmd) 
	if(not cmd or string.len(cmd)==0) then
		...
		if(TFW_SHOW) then
			if GetNumRaidMembers()>5 then TFW_AUTO = "force" end
...
local function LFW_OnEvent(self,event,...) 
	...
	elseif(event=="CHAT_MSG_CHANNEL_NOTICE") then
		...
		elseif(channelName==TFW_CHANNEL_NAME) then
			...
			elseif(message == "BANNED") then
				TFW_AddMessage(format(_G["CHAT_"..arg1.."_NOTICE"], arg9), nil, nil, nil, true)
				kicked = 60
			end
		else
	...
	elseif(event=="CHAT_MSG_CHANNEL_NOTICE_USER") then  		--Response to DisplayChannelOwner(TFW_CHANNEL_NAME);
		...
		elseif(message=="PLAYER_BANNED") then
			TFW_AddMessage(format(_G["CHAT_"..arg1.."_NOTICE"], arg8, arg4, arg2, arg5), nil, nil, nil, true)
...
SocialsMicroButton:SetScript("OnClick", function () 
...
hooksecurefunc("UpdateMicroButtons", function()
	if ( not TFW_SHOW ) then
		SocialsMicroButton:SetAlpha(0.25)
	else
